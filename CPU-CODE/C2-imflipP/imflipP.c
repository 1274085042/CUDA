...
void FlipImageV(unsigned char** img)
{
	...
	//vertical flip
	for(col=0; col<ip.Hbytes; col+=3) // Hbytes每一行的字节数
	{
		row = 0; 
		while(row<ip.Vpixels/2)      // 图像的高
		{
			pix.B = img[row][col];
			pix.G = img[row][col+1];
			pix.R = img[row][col+2];
...
{
	struct Pixel pix; //temp swap pixel 临时交换像素
	int row, col;
	...
	pthread_exit(0);			//告知main函数，子线程已经完成
}
...
int main(int argc, char** argv)
{
	...
	if(NumThreads >1){
		...
		for(a=0; a<REPS; a++){
			...
			for(i=0; i<NumThreads; i++){
				// 等待线程执行完，操作系统回收线程的资源（内存、虚拟CPU和栈）
